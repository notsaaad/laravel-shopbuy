...
@section('content')
<div class="only-form">
  <form action="{{ route('attribute.store') }}"  method="post" class="add_user">
    @csrf
      <div class="two-input">
        <div class="input-div w-half">
          <label for="user_username" class="riq">name</label>
          <input type="text" autocomplete="off" value="{{ old('name') }}" id="name" name="name" placeholder="Enter Attribute name">
          @error('name')
